...
class RetinaNetHead(mx.gluon.nn.HybridBlock):
    def __init__(self, num_classes, num_anchors):
        ...
        with self.name_scope():
            ...
            for i in range(4):
                ...
                # There is not GroupNorm here in the original implementation.
...
class RetinaNet(mx.gluon.nn.HybridBlock):
    def __init__(self, backbone, num_classes, num_anchors):
        super(RetinaNet, self).__init__()
        self.backbone = backbone
        self._head = RetinaNetHead(num_classes, num_anchors)

    def hybrid_forward(self, F, x):
        # typically the strides are (4, 8, 16, 32, 64)
        x = self.backbone(x)
        outputs = [self._head(xx) for xx in x]
...
class PyramidNeckRetinaNet(mx.gluon.nn.HybridBlock):
    ...
    def hybrid_forward(self, F, res2, res3, res4, res5):
        ...
        P6 = self.fpn_p6_3x3(res5)  # FCOS use P5 instead of C5.
...
def train_net(config):
    ...
    net = RetinaNet(backbone, config.dataset.NUM_CLASSES, num_anchors)
    ...
    if config.TRAIN.aspect_grouping:
        ...
        train_loader = mx.gluon.data.DataLoader(dataset=train_dataset, batch_size=1, batchify_fn=batch_fn,
                                                num_workers=24, last_batch="discard", shuffle=True, thread_pool=False)
    else:
        assert False
    ...
    for epoch in range(config.TRAIN.begin_epoch, config.TRAIN.end_epoch):
        for nbatch, data_batch in enumerate(tqdm.tqdm(train_loader, total=len(train_loader), unit_scale=1)):
            ...
            #         reg_fpn_predictions, cls_fpn_predictions = net(data)
            #         reg_fpn_predictions = reg_fpn_predictions.reshape((0, 4, num_anchors, -1))
            #         cls_fpn_predictions = cls_fpn_predictions.reshape((0, cls_fpn_predictions.shape[1] // num_anchors, num_anchors, -1))
            #
...
def inference_one_image(config, net, ctx, image_path):
    ...
    for reg_prediction, cls_prediction, base_size, stride in zip([x[0] for x in fpn_predictions],
        ...
        topk_indices = mx.nd.topk(cls_prediction.reshape(0, -1), axis=1, ret_typ="indices", k=200)
        rois = mobula.op.RetinaNetRegression(data,
                                             reg_prediction,
                                             cls_prediction,
                                             topk_indices,
