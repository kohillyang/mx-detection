...
def _add_conv(out, channels=1, kernel=1, dilate=1, stride=1, pad=0,
              num_group=1, active=True, relu6=False, norm_layer=BatchNorm, norm_kwargs=None):
    out.add(nn.Conv2D(channels, kernel, stride, pad, groups=num_group, use_bias=False, dilation=dilate))
    out.add(norm_layer(scale=True, **({} if norm_kwargs is None else norm_kwargs)))
    if active:
        out.add(ReLU6() if relu6 else nn.Activation('relu'))


def _add_conv_dw(out, dw_channels, channels, stride, dilate, relu6=False,
                 norm_layer=BatchNorm, norm_kwargs=None):
    _add_conv(out, channels=dw_channels, kernel=3, dilate=dilate, stride=stride,
              pad=int((3-1 + 2 * (dilate-1))/2), num_group=dw_channels, relu6=relu6,
        ...
        with self.name_scope():
            ...
            with self.features.name_scope():
                ...
                channels = [int(x * multiplier) for x in [64] +
                            [128] * 2 + [256] * 2 + [512] * 6 ]
                strides = [1, 2] * 2 + [1, 1] + [1] * 5
                dilates = [1, 1] * 2 + [2] * 7
                for dwc, c, s, d in zip(dw_channels, channels, strides, dilates):
                    _add_conv_dw(self.features, dw_channels=dwc, channels=c, stride=s, dilate=d,
                                 norm_layer=norm_layer, norm_kwargs=norm_kwargs)
...
def get_mobilenet(multiplier, pretrained=False, ctx=cpu(),
    ...
    if pretrained:
        ...
        net.load_parameters(get_model_file('mobilenet%s' % version_suffix,
                                           tag=pretrained,
                                           root=root), ctx=ctx, ignore_extra=True)
