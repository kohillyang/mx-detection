...
class DeformabelConv2D(nn.HybridBlock):
    def __init__(self, num_filter, kernel_size, strides, padding, dilation, use_bias, in_channels):
        ...
        with self.name_scope():
            self.conv_offset = nn.Conv2D(channels=72, kernel_size=kernel_size, strides=strides, dilation=dilation,
                                         padding=padding, weight_initializer=init, bias_initializer=init, prefix="offset_")

            init = mx.init.Normal()
            init.set_verbosity(True)
            self.weight = self.params.get('weight', shape=(num_filter, in_channels, kernel_size, kernel_size),
                                          init=init,
...
# Helpers
def _conv3x3(channels, stride, in_channels, use_dcn, use_dilated_dcn):
    if use_dcn:
        padding, dilation = (2, 2) if use_dilated_dcn else (1, 1)
        return DeformabelConv2D(channels, kernel_size=3, strides=stride, padding=padding, dilation=dilation,
...
class BottleneckV1(HybridBlock):
    ...
    def __init__(self, channels, stride, downsample=False, in_channels=0,
                 last_gamma=False, use_se=False, use_dcn=False, use_dilated_dcn=False, **kwargs):
        super(BottleneckV1, self).__init__(**kwargs)
        self.body = nn.HybridSequential(prefix='')
        self.body.add(nn.Conv2D(channels//4, kernel_size=1, strides=stride))
        self.body.add(nn.BatchNorm(use_global_stats=True))
        self.body.add(nn.Activation('relu'))
        self.body.add(_conv3x3(channels//4, 1, channels//4, use_dcn=use_dcn, use_dilated_dcn=use_dilated_dcn))
        self.body.add(nn.BatchNorm(use_global_stats=True))
        self.body.add(nn.Activation('relu'))
...
class ResNetV1(HybridBlock):
        ...
        with self.name_scope():
            ...
            for i, num_layer in enumerate(layers):
                ...
                self.features.add(self._make_layer(block, num_layer, channels[i+1],
                                                   ...
                                                   use_dcn=i in [1, 2, 3], use_dilated_dcn=False
                                                   ))
    ...
    def _make_layer(self, block, layers, channels, stride, stage_index, in_channels=0,
                    last_gamma=False, use_se=False, use_dcn=False, use_dilated_dcn=False):
        layer = nn.HybridSequential(prefix='stage%d_'%stage_index)
        with layer.name_scope():
            layer.add(block(channels, stride, channels != in_channels, in_channels=in_channels,
                            last_gamma=last_gamma, use_se=use_se, prefix='', use_dcn=use_dcn, use_dilated_dcn=use_dilated_dcn))
