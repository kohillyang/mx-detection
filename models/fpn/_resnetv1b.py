...
def _conv3x3(in_channels, num_filter, kernel_size, strides, padding, dilation, use_bias=False):
    return DeformabelConv2D(num_filter=num_filter, kernel_size=kernel_size, strides=strides, padding=padding,
                            dilation=dilation, use_bias=use_bias, in_channels=in_channels)
...
class BottleneckV1b(HybridBlock):
    ...
    def __init__(self, planes, strides=1, dilation=1,
                 ...
                 norm_kwargs=None, last_gamma=False, use_dcn=False, **kwargs):
        ...
        self.relu1 = nn.Activation('relu')
        if use_dcn:
            self.conv2 = _conv3x3(in_channels=planes, num_filter=planes, kernel_size=3, strides=strides,
                                  padding=dilation, dilation=dilation, use_bias=False)
        else:
    ...
    def __init__(self, block, layers, classes=1000, dilated=False, norm_layer=BatchNorm,
        ...
        with self.name_scope():
            ...
            self.layer2 = self._make_layer(2, block, 128, layers[1], strides=2, avg_down=avg_down,
                                           norm_layer=norm_layer, last_gamma=last_gamma, use_dcn=True)
            if dilated:
