...
from ._resnext import se_resnext50_32x4d, se_resnext101_64x4d


class SEResNext101_64x4d(mx.gluon.nn.HybridBlock):
    def __init__(self, feature_dim=256, pretrained=True):
        super(SEResNext101_64x4d, self).__init__(prefix="se_resnext101_64x4d_")
        self.eps = 1e-5
        feat = se_resnext101_64x4d(pretrained=True, norm_kwargs={"use_global_stats":True})
        self.feat = feat
...
class SEResNext50_32x4d(mx.gluon.nn.HybridBlock):
    def __init__(self, feature_dim=256, pretrained=True):
        super(SEResNext50_32x4d, self).__init__(prefix="se_resnext50_32x4d_")
        self.eps = 1e-5
        feat = se_resnext50_32x4d(pretrained=True, norm_kwargs={"use_global_stats":True})
        self.feat = feat
        self.mean = self.params.get('mean', shape=[1, 3, 1, 1],
                                    init=mx.init.Zero(),
                                    allow_deferred_init=False, grad_req='null')
