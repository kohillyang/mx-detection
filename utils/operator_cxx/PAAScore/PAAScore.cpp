...
MOBULA_FUNC void paa_score(
		...
		T* output) {
	printf("%d %d %d %d %d %d %d %d %d\n", stride, image_h, image_w, nbatch, feature_h, feature_w, number_of_anchors, number_of_classes, gt_boxes_padded_length);
	...
	parfor(nbatch * number_of_anchors * feature_h * feature_w, [&](int index) {
		...
		Tensor5D<T> tensor_output = Tensor5D<T>(output, nbatch, number_of_classes, number_of_anchors, feature_h, feature_w);
		int total_idx = index;
        ...
        T net_pred_0 = tensor_reg_preds(batch_idx, 0, anchor_idx, h_idx, w_idx) * .1;
        T net_pred_1 = tensor_reg_preds(batch_idx, 1, anchor_idx, h_idx, w_idx) * .1;
        T net_pred_2 = tensor_reg_preds(batch_idx, 2, anchor_idx, h_idx, w_idx) * .2;
        T net_pred_3 = tensor_reg_preds(batch_idx, 3, anchor_idx, h_idx, w_idx) * .2;
