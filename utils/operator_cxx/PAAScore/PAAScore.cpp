...
MOBULA_KERNEL paa_score_kernel(
	...
	T *pointer_bbox_norm_coef,
	T* output) {

	T bbox_std0 = pointer_bbox_norm_coef[0];
	T bbox_std1 = pointer_bbox_norm_coef[1];
	T bbox_std2 = pointer_bbox_norm_coef[2];
	T bbox_std3 = pointer_bbox_norm_coef[3];
	...
	parfor(nbatch * number_of_anchors * feature_h * feature_w, [&](int index) {
		Tensor5D<T> tensor_reg_preds = Tensor5D<T>(pointer_reg_preds, nbatch, 4, number_of_anchors, feature_h, feature_w);
		Tensor5D<T> tensor_cls_preds = Tensor5D<T>(pointer_cls_preds, nbatch, number_of_classes, number_of_anchors , feature_h, feature_w);
		Tensor3D<T> tensor_gt_boxes = Tensor3D<T>(pointer_gt_boxes, nbatch, gt_boxes_padded_length, 5);
		Tensor5D<T> tensor_output = Tensor5D<T>(output, nbatch, 2, number_of_anchors, feature_h, feature_w);
		int total_idx = index;
		...
		T net_pred_0 = tensor_reg_preds(batch_idx, 0, anchor_idx, h_idx, w_idx) * bbox_std0;
		T net_pred_1 = tensor_reg_preds(batch_idx, 1, anchor_idx, h_idx, w_idx) * bbox_std1;
		T net_pred_2 = tensor_reg_preds(batch_idx, 2, anchor_idx, h_idx, w_idx) * bbox_std2;
		T net_pred_3 = tensor_reg_preds(batch_idx, 3, anchor_idx, h_idx, w_idx) * bbox_std3;
		...
		// For each anchor, the paa score is the max iou with all gt_boxes times the class score.
		T max_iou = 0;
		...
		if(max_iou_gt_box_idx >= 0 && max_iou > 0){
			...
			tensor_output(batch_idx, 0, anchor_idx, h_idx, w_idx) = max_iou * net_score_pred; // paa score
			tensor_output(batch_idx, 1, anchor_idx, h_idx, w_idx) = gt_box_idx; // To indicate the anchor assigned which groundtruth box.
		}
	}); // parfor
